    ...
    .then(function (visDoc) {
      ...
      activeListener = msgRef.onSnapshot(function (snapshot) {
        ...
        snapshot.forEach(function (doc) {
          ...
          row.innerHTML =
            avEl +
            "<div class='msg-bubble' id='bubble_" +
            docId +
            ...
            seenHTML +
            "</div>" +
            "<div class='msg-actions-toolbar' id='toolbar_" +
            docId +
            "'>" +
...
document.addEventListener("visibilitychange", function () {
  if (document.visibilityState === "hidden") {
    stopHeartbeat();
    setPresenceOff();
  } else if (currentUser) startHeartbeat();
});
window.addEventListener("online", function () {
  if (currentUser) startHeartbeat();
});
...
function toggleStatsPanel() {
  ...
  panel.classList.toggle("open", _statsPanelOpen);
  if (_statsPanelOpen) {
...
function loadStats() {
  ...
  db.collection("recentChats")
    ...
    .then(function (snap) {
      var sent = 0,
        ...
        unread = 0,
        p = [];
      set("statTotalChats", snap.size);
      snap.forEach(function (doc) {
        var cid = doc.data().chatID || "";
        if (!cid) return;
        p.push(
          db
            .collection("conversations")
            ...
            .then(function (msgs) {
              msgs.forEach(function (m) {
                var md = m.data();
                if (md.senderUID === uid) sent++;
                else recv++;
                if (md.reactions) react += Object.keys(md.reactions).length;
                if (md.seenBy && !md.seenBy[uid] && md.senderUID !== uid)
                  unread++;
              });
            })
            .catch(function () {}),
      ...
      Promise.all(p).then(function () {
        set("statTotalSent", sent);
        set("statTotalReceived", recv);
        set("statTotalReactions", react);
        set("statUnread2", unread);
      });
    })
    .catch(function () {});
  db.collection("groups")
  ...
  db.collection("recentChats")
    ...
    .then(function (snap) {
      ...
      Promise.all(p).then(function () {
        set("statDailyMessages", daily);
      });
    })
    .catch(function () {});
}
function translateMessage(msgId, text) {
  if (!text || !text.trim()) {
    showToast("No text to translate.", "error");
    return;
  }
  var bar = document.getElementById("trans_" + msgId);
  if (bar) {
    bar.classList.toggle("show");
    return;
  }
  var bubble = document.getElementById("bubble_" + msgId);
  if (!bubble) {
    showToast("Translation unavailable.", "error");
    return;
  }
  var tb = document.createElement("div");
  tb.className = "trans-bar show";
  tb.id = "trans_" + msgId;
  tb.innerHTML =
    "<div class='trans-bar-label'>🌐 Urdu</div><div class='trans-bar-text trans-loading'>Translating…</div>";
  ...
  fetch(
    ...
    .then(function (d) {
      var t =
        d.responseData && d.responseData.translatedText
          ? d.responseData.translatedText
          : "Unavailable";
      var el = tb.querySelector(".trans-bar-text");
      if (el) {
...
document.addEventListener("click", function (e) {
  ...
  if (
    !e.target.closest(".msg-bubble") &&
    !e.target.closest(".msg-actions-toolbar")
  ) {
    if (_activeToolbar) {
      _activeToolbar.classList.remove("mobile-show");
      _activeToolbar = null;
    }
  }
});
...
function showToolbar(docId, type, bubbleEl) {
  _tbDocId = docId;
  var tb = document.getElementById("globalToolbar");
  if (!tb) return;
  var row = document.getElementById("msgrow_" + docId);
  var isMine = row && row.getAttribute("data-ismine") === "1";
